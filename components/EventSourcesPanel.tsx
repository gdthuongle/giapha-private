...
export function EventSourcesPanel({
  ...
  const handleLinkExisting = () => {
    ...
    startTransition(async () => {
      const result = await linkExistingEventSource({
        eventId,
  ...
  const handleUpdate = () => {
    ...
    startTransition(async () => {
      const result = await updateEventSourceLink({
        eventId,
    ...
    <section className="mb-5 rounded-2xl border border-amber-200 bg-amber-50/60 p-4">
      <div className="mb-4 flex items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-stone-600">
            {eventTitle
              ? `Sự kiện: ${eventTitle}`
              : "Liên kết nguồn xác minh cho sự kiện này."}
          </p>
        </div>
      ...
      {editingSource ? (
        <div className="mb-5 grid gap-3 rounded-xl border border-blue-100 bg-blue-50/60 p-3">
          <div className="flex items-center justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-xs text-stone-500">
                Thông tin nguồn là dùng chung; trích dẫn và ghi chú riêng chỉ áp dụng cho sự kiện này.
              </p>
            </div>
            <button
              type="button"
          ...
          <label className="grid gap-1 text-sm">
            <span className="font-medium text-stone-700">Trích dẫn riêng cho sự kiện này</span>
            <textarea
          ...
          <label className="grid gap-1 text-sm">
            <span className="font-medium text-stone-700">Ghi chú riêng cho sự kiện này</span>
            <textarea
              value={editLinkNote}
      ...
      {showForm ? (
        <div className="mb-5 rounded-xl border border-amber-100 bg-white/80 p-3">
